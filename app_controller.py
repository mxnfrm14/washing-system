...
# Alternative approach if you prefer to keep windows hidden instead of destroying them
class AppControllerAlternative:
    """
    Alternative controller that hides/shows windows instead of destroying them
    """
    def __init__(self):
        # Initialize custom themes and appearance
        ctk.set_appearance_mode("System")
    ...
    def show_main_app(self):
        ...
        # Create the main app if it doesn't exist
        if self.main_app is None:
            self.main_app = App()
            self.main_app.protocol("WM_DELETE_WINDOW", self.on_main_app_close)
        
        # Refresh all custom button icons
        CustomButton.refresh_all_icons()
    ...
    def on_main_app_close(self):
        ...
        # Refresh icons for welcome window
        CustomButton.refresh_all_icons()
